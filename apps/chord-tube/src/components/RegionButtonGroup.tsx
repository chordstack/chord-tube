...
import { useMediaQuery, Select, MenuItem, FormControl, InputLabel, Box } from '@mui/material';
...
const regions = [
    ...
    { code: 'IN', label: 'IN', flag: 'https://flagcdn.com/in.svg' },
];
...
export default function RegionButtonGroup() {
    ...
    const handleChange = (event: React.ChangeEvent<{ value: unknown }>) => {
        setRegionCode(event.target.value as string);
    };
    ...
    return isMobile ? (
                ...
                {regions.map(({ code, label }) => (
                    <MenuItem key={code} value={code}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 3 }}>
                            <img src={`https://flagcdn.com/${code.toLowerCase()}.svg`} alt={label} className=" h-5 w-6 object-cover" />
                            {label}
                        </Box>
                    </MenuItem>
                ))}
        ...
        >
            <h1 className='font-medium text-lg'>Region :</h1>
            {regions.map(({ code, label, flag }) => (
                ...
                >
                    <img src={flag} alt={label} className="w-5 h-5 object-cover rounded-sm" />
                    <span>{label}</span>
                </ToggleButton>
            ))}
        </ToggleButtonGroup>
